    ...
    override fun onStart() {
        ...
        btnRegister.setOnClickListener {
            if(!acceptTerms.isChecked){
                Toast.makeText(applicationContext, "Aceite os termos de Serviço antes de continuar!", Toast.LENGTH_SHORT).show()
                return@setOnClickListener
            }
            if (name.text.toString().isBlank() || email.text.toString().isBlank() || password.text.toString().isBlank() || confirmPassword.text.toString().isBlank()) {
            ...
            val verifyEmail = sharedPrefs.getString("user_${email.text}", null)
            if(verifyEmail != null){
                Toast.makeText(this, "Email já cadastrado!", Toast.LENGTH_SHORT).show()
                return@setOnClickListener
            }
            val role = "user"
            editor.putString("user_${email.text}", "${password.text},$role")
            editor.putString("name_${email.text}", name.text.toString())
            editor.apply()
